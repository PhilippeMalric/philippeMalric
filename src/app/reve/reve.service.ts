...
export class ReveService {
  ...
  getItems = ()=>{
    ...
    return this.afs.collection("reve")
            .valueChanges()
  ...
  deleteMessage(messageid:string) {
    ...
    return from(this.afs.doc(`reve/${messageid}`).delete());
}

updateMessage(messageid:string, changes: Partial<Message>):Observable<any> {
    return from(this.afs.doc(`reve/${messageid}`).update(changes));
}
...
createMessage(newMessage: Partial<Message>, messageid:string) {
    return this.afs.collection("reve",
        ...
        .pipe(
            concatMap(result => {
console.log("inside createMessage2 reve")
                ...
                save$ = from(this.afs.doc(`reve/${messageid}`).set(message));
