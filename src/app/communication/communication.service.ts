...
export class CommunicationService {
  ...
  deleteMessage(messageid:string) {
    ...
    return from(this.afs.doc(`items/${messageid}`).delete());
}

updateMessage(messageid:string, changes: Partial<Message>):Observable<any> {
    return from(this.afs.doc(`items/${messageid}`).update(changes));
}
...
console.log("inside createMessage2")
                ...
                save$ = from(this.afs.doc(`items/${messageid}`).set(message));
