...
import { TresorService, Tresor } from '../tresor.service';
...
export class RacineComponent implements OnInit {
  ...
  constructor(private tresorService:TresorService,
  ...
  ngOnInit(): void {
    ...
    this.id = this.tresorService.create_Id()
    this.tresorService.getItems().pipe().subscribe((data)=>{
      console.log("data")
      console.log(data)
  ...
  add(){
    ...
    this.userService.userName.pipe(take(1)).subscribe((data)=>{
      ...
      this.tresorService.createMessage(
        {auteur:data,
          type:"message",
  ...
  delete(e:Tresor){
    ...
    this.tresorService.deleteMessage(e.id_firestore).subscribe()

  }

  delete2(e:Tresor){

    this.tresorService.falsedeleteMessage(e.id_firestore,e).subscribe()
